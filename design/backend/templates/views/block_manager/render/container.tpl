        ...
        <div class="grid-control-menu bm-control-menu">
            ...
            {if $container.has_displayable_content && !$dynamic_object}
                <div class="grid-control-menu-actions">
                    ...
                    <a href="#" class="dropdown-toggle cm-tooltip" data-toggle="dropdown" data-ce-tooltip-position="top" title="{__("insert_grid")}">{include_ext file="common/icon.tpl" source="plus"}</a>
                        <ul class="dropdown-menu droptop">
                            <li><a href="#" class="cm-action bm-action-add-grid">{__("insert_grid")}</a></li>
                        </ul>
                    </div>
                    <div class="cm-tooltip cm-action bm-action-properties action" data-ce-tooltip-position="top" title="{__("container_options")}">{include_ext file="common/icon.tpl" source="cog"}</div>
                    <div class="cm-action bm-action-switch cm-tooltip action" data-ce-tooltip-position="top" title="{__("enable_or_disable_container")}">{include_ext file="common/icon.tpl" source="off"}</div>
                </div>
            {/if}
        </div>
