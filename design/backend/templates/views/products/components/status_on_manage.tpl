...
{if $non_editable || $display == "text"}
    <span class="view-status view-status-{$status|lower} view-status--type view-status--type-products-manage">
    ...
    <div class="cm-popup-box {if !$hide_for_vendor}dropdown{/if} dropleft {$popup_additional_class}"
        ...
        {if !$hide_for_vendor}
            <a href="#"
                ...
                class="{$btn_meta} btn dropdown-toggle link--monochrome {if $id}cm-combination{/if} status-dropdown status-dropdown-{$status|lower} status-dropdown--type status-dropdown--type-products-manage"
                ...
                {foreach $items_status as $status_id => $status_name}
                    ...
                    <li {if $status == $status_id}class="disabled"{/if}>
                        {hook name="products:status_select_item"}
                            <a class="status-link status-link-{$status_id|lower} status-link--type status-link--type-products-manage {$status_meta} {if $confirm}cm-confirm{/if} {if $status == $status_id}active{else}cm-ajax cm-post {if $ajax_full_render}cm-ajax-full-render{/if}{/if}"
                            {if $status_target_id}
