<tr class="multiple-table-row">
    <td>
        ...
        <a class="row-status link--monochrome cm-external-click{if $non_editable} no-underline{/if}" data-ca-external-click-id="opener_group_folder_{$id}">{$folder.folder_name}</a></td>
    ...
    <td width="15%">
        <div class="pull-right nowrap">
            ...
            {/if}
            {include file="common/select_popup.tpl" type="product_file_folders" popup_additional_class="dropleft" id=$id status=$folder.status hidden=$hidden object_id_name='folder_id' table="product_file_folders" hide_for_vendor=$hide_for_vendor display=$display non_editable=$non_editable}
        </div></td>
</tr>