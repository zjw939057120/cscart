                ...
                {if !$nostatus}
                    <td width="{$last_col_width|default:"12%"}" {if $status_row_hint}data-th="{$status_row_hint}"{else}data-th="&nbsp;"{/if}>
                        <div class="pull-right nowrap">
                            ...
                            {include file="common/select_popup.tpl" type=$table popup_additional_class="dropleft" id=$id status=$status hidden=$hidden object_id_name=$object_id_name table=$table hide_for_vendor=$hide_for_vendor display=$display non_editable=$non_editable update_controller=$update_controller st_result_ids=$st_result_ids}
                        </div>
                    </td>
                {/if}
            <!--{$row_id}--></tr>
