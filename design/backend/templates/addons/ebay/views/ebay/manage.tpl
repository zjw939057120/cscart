        ...
        <div class="table-responsive-wrapper longtap-selection">
            ...
            >
                ...
                <td width="45%" class="row-status" data-th="{__("name")}">
                    <a href="{"ebay.update?template_id=`$template.template_id`"|fn_url}" class="link--monochrome">{$template.name}</a>
                    {include file="views/companies/components/company_name.tpl" object=$template}
                </td>
                <td width="15%" data-th="{__("products")}">
                    <a href="{"products.manage?ebay_template_id=`$template.template_id`"|fn_url}" class="link--monochrome">{$template.product_count}</a>
                </td>
                ...
                <td width="10%" class="right nowrap" data-th="{__("status")}">
                    {include file="common/select_popup.tpl" type="ebay_templates" popup_additional_class="dropleft `$no_hide_input`" display=$display id=$template.template_id status=$template.status hidden=false object_id_name="template_id" table="ebay_templates"}
                </td>
            </tr>
            {/foreach}
            </table>
...
{capture name="adv_buttons"}
    {include file="common/tools.tpl"
        ...
        prefix="top"
        hide_tools="true"
        title=__("add_ebay_template")
        link_text=__("add_ebay_template")
        icon="icon-plus"
    }
{/capture}
