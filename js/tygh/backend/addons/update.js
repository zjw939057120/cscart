(function (_, $) {
  ...
  $.ceEvent('on', 'ce.tab.show', function (tab_id, $tabs_elm) {
    ...
    let isSettingsTabActive = !$('#content_settings').hasClass('hidden');
    $('.cm-addons-save-settings').toggleClass('hidden', !isSettingsTabActive);

    // Toggle subscription save button
    let isSubscriptionTabActive = !$('#content_subscription').hasClass('hidden');
