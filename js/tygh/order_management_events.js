(function (_, $) {
  ...
  $.ceEvent('on', 'ce.formcheckfailed_om_cart_form', function ($form) {
    const $elemHavingError = $form.find('.order-management-options-content .control-group.error'),
      hasError = $elemHavingError.length > 0;
    if (hasError) {
      $elemHavingError.each(function () {
        const $parent = $(this).closest('td'),
          cart_id = $parent.find('[name="appearance[id]"]').val(),
          product_id = $parent.find("[name=\"cart_products[".concat(cart_id, "][product_id]\"]")).val();

        //expand the list of options if validation error
