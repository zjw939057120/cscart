(function (_, $) {
  ...
  const methods = {
    ...
    createEditor: function (e) {
      const $liveEditValueWrapper = $(e.target);
      if (!$liveEditValueWrapper.closest('.cm-icon-live-edit').length) {
        return;
      }
      let targetElement = $('.cm-live-edit', $liveEditValueWrapper.closest('var')).first();
      ...
      const phrase = methods._getPhrase(targetElement),
      ...
      let inputTemplate = inputByTypeMap[liveEditorObject.type] || inputByTypeMap.textarea,
      ...
      $dialog.ceDialog('open', dialogParams);

      // Init wysiwyg
      if ($editorInput.hasClass('cm-wysiwyg')) {
        $editorInput.ceEditor();
        setTimeout(() => $dialog.ceDialog('reload'), 300);
      }
    ...
    load: function (content) {
      $('[data-ca-live-editor-obj]', content).each(function () {
        ...
        if (currentValue) {
          const currentValueEscaped = $('<div>').text(currentValue).html();
    ...
    init: function () {
      ...
      $(_.doc).on('click', '.ty-live-editor__closer', () => {
        $('.ty-live-editor').ceDialog('close');
      });
      $(_.doc).on('click', '.ty-live-editor__saver', () => {
