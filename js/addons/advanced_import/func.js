(function (_, $) {
  ...
  var methods = {
    ...
    toggleXmlOptions: function (file_extension) {
      if (!file_extension || file_extension !== 'xml') {
    ...
    removeCompanyIdInputError: function () {
      const $companyInput = $('#elm_company_id');
      if (!$companyInput.length) {
        return;
      }
      if ($companyInput.val()) {
        $companyInput.removeClass('cm-failed-field');
        const $companyInputControlGroup = $companyInput.closest('.control-group');
        ...
        if (presetField) {
          ...
          if (relatedObjectType && relatedObject) {
            ...
            if (!object) {
              $.ceNotification('show', {
                title: _.tr('warning'),
                message: _.tr('advanced_import.some_product_properties_not_found'),
                type: 'W'
              });
              return;
            }
