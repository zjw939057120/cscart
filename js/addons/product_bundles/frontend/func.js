(function (_, $) {
  function submitBundleForm($form) {
    ...
    var $optionsRequired = $('[data-ca-product-bundles="optionsRequired"]', $targetForm);

    // Create a temporary container for the fields in the form
    var $formData = $('<div>').attr({
      'data-ca-product-bundles': 'formData',
      class: 'hidden'
    });
    $formInner.append($formData);

    // Move fields to the temporary container
    $(':input:enabled[name]', $form.find('[data-ca-product-bundles="fieldContainer"]')).appendTo($formData);

    // Rename "Product ID" to "Product ID + Bundle product key"
  ...
  function fixScrollerWidthInPopup(scrollerData, stage) {
    const $body = $('body');
    const isMobile = $body.hasClass('screen--xs') || $body.hasClass('screen--xs-large') || $body.hasClass('screen--sm');
    const $scroller = scrollerData.$elem;
    let $bundleDialog = {};
